				 ...
				 //  because there is a chance user can generate twenty-one consecutive 1's
	{
		...
		if (p <= 21) //If player total is less than 21, ask to hit or stay
		{
			...
			while (1)
			{
				...
				else if ((time(0) - be) == 17 && n == 0)
				{
					printf("\nYou have %d seconds left\n", 3);
					n = 1;
				}
				else if ((time(0) - be) == 18 && n == 1)
				{
					printf("You have %d seconds left\n", 2);
					n = 2;
				}
				else if ((time(0) - be) == 19 && n == 2)
				{
					printf("You have %d seconds left\n", 1);
					n = 3;
				}
				else if ((time(0) - be) >= 20)
				{
					be = time(0);
					n = 0;
					choice3 = 's';
...
void askover() // Function for asking player if they want to play again
{
	...
	{
		fileresults();
		printf("\nWould you want to join the leaderboard?");
		printf("\nPlease Enter Y for Yes or N for No\n");
		scanf("\n%c", &choice1);
		if ((choice1 == 'Y') || (choice1 == 'y')) // If yes, continue.
		{
...
bool sortbyscore(pair<string, int> a, pair<string, int> b) {
	// 降冪排序副程式
	return a.second > b.second;
}

void boardsort() {
	ifstream file("board.txt"); // 讀txt檔
	string s;                   // 用來存放從txt檔讀取的每一行的字串
	vector<pair<string, int>> names_scores_pairs;

	while (getline(file, s)) {
		stringstream ss(s); // 分割字串（white space）
		string name;
		int score;

		ss >> name >> score; // 分割字串（white space）

		names_scores_pairs.push_back({ name, score }); // 將name和score成對地存入vector

		sort(names_scores_pairs.begin(), names_scores_pairs.end(), sortbyscore); // 降冪排序
	};
	cout << endl << "Name" << " " << "score" << endl << "--------------------------" << endl;
	for (auto i : names_scores_pairs) {
